const blogs = [
  {
    id: 1,
    title: "Exploring the Beaches of Bali",
    image: "/images/photo.jpg.avif",
    description:
      "Bali offers pristine beaches, vibrant culture, and breathtaking sunsets. Discover the best spots to relax and explore...",
    content: `Bali is a paradise for beach lovers and adventure seekers alike. 

Top things to do:
1. Relax at Kuta and Seminyak beaches
2. Visit Uluwatu Temple for sunset views
3. Try surfing at Canggu Beach
4. Explore rice terraces in Ubud

Travel Tips:
- Best time to visit: April to October
- Respect local customs and dress codes
- Rent a scooter for convenient travel

Don’t forget to enjoy local Balinese cuisine like Nasi Goreng and Babi Guling.`,
    author: "Alice Traveler",
    date: "2025-01-12",
    category: "Travel",
  },
  {
    id: 2,
    title: "A Weekend in Paris: Must-See Attractions",
    image: "/images/photo.jpg.avif",
    description:
      "Paris, the city of love, is filled with historic landmarks, art, and amazing cuisine. Here's how to make the most of a short visit...",
    content: `Paris has something for every traveler—from art lovers to food enthusiasts.

Highlights:
- Eiffel Tower and Seine river cruise
- Louvre Museum and Musée d'Orsay
- Stroll through Montmartre and visit Sacré-Cœur
- Enjoy French pastries at local cafes

Travel Tips:
- Purchase museum tickets in advance
- Use the metro for easy transportation
- Try local specialties like croissants and escargot

Paris is best explored by walking its charming streets and enjoying the café culture.`,
    author: "Jean Dupont",
    date: "2025-01-15",
    category: "Travel",
  },
  {
    id: 3,
    title: "Hiking the Swiss Alps: An Adventure Guide",
    image: "/images/photo.jpg.avif",
    description:
      "The Swiss Alps are perfect for hikers and nature lovers. Experience stunning mountain trails, scenic villages, and alpine lakes...",
    content: `Hiking in the Swiss Alps is an unforgettable experience.

Top Trails:
- Zermatt to Gornergrat
- Jungfrau region
- Lauterbrunnen valley hikes
- Lake Oeschinen trails

Travel Tips:
- Check weather conditions before hiking
- Wear proper hiking gear
- Stay in mountain lodges for authentic experience
- Carry a map and water

Enjoy panoramic views, fresh mountain air, and cozy alpine cuisine along the way.`,
    author: "Liam Mountain",
    date: "2025-01-18",
    category: "Adventure",
  },
  {
    id: 4,
    title: "Discovering Kyoto’s Ancient Temples",
    image: "/images/photo.jpg.avif",
    description:
      "Kyoto, Japan’s cultural heart, is home to historic temples, beautiful gardens, and traditional tea houses...",
    content: `Kyoto is perfect for those seeking history and serenity.

Must-visit Temples:
- Kinkaku-ji (Golden Pavilion)
- Fushimi Inari Shrine
- Ryoan-ji Zen Garden
- Gion district for traditional tea houses

Travel Tips:
- Purchase a transportation pass for buses/trains
- Respect temple etiquette
- Visit during cherry blossom season for stunning views

Kyoto offers a perfect blend of tradition, tranquility, and exquisite Japanese cuisine.`,
    author: "Sakura Tanaka",
    date: "2025-01-20",
    category: "Culture",
  },
  {
    id: 5,
    title: "Road Trip Along California’s Pacific Coast Highway",
    image: "/images/photo.jpg.avif",
    description:
      "Drive along one of the most scenic coastal roads in the world, enjoying ocean views, charming towns, and iconic landmarks...",
    content: `The Pacific Coast Highway (PCH) is a dream road trip.

Highlights:
1. Start in San Francisco: Golden Gate Bridge, Fisherman’s Wharf
2. Santa Cruz beaches
3. Big Sur cliffs and McWay Falls
4. Santa Barbara for wine and relaxation
5. End in Los Angeles: Venice Beach and Hollywood

Travel Tips:
- Rent a car for flexibility
- Plan for stops at scenic viewpoints
- Bring a camera for incredible photos
- Book accommodations in advance during peak season

Enjoy the freedom of the open road and the breathtaking California coastline.`,
    author: "Chris Roadster",
    date: "2025-01-22",
    category: "Travel",
  },
  {
    id: 6,
    title: "Exploring the Wonders of Iceland",
    image: "/images/photo.jpg.avif",
    description:
      "Iceland is a land of waterfalls, glaciers, geysers, and volcanic landscapes. Learn the top spots and travel tips for adventure seekers...",
    content: `Iceland is a paradise for nature enthusiasts.

Must-See Attractions:
1. Golden Circle: Thingvellir, Geysir, Gullfoss
2. Blue Lagoon geothermal spa
3. Jokulsarlon Glacier Lagoon
4. Northern Lights in winter

Travel Tips:
- Rent a 4x4 vehicle for rural exploration
- Pack warm, waterproof clothing
- Respect fragile ecosystems
- Book accommodations in advance

From dramatic landscapes to unique wildlife, Iceland offers a once-in-a-lifetime experience.`,
    author: "Olivia Frost",
    date: "2025-01-25",
    category: "Adventure",
  }
];

export default blogs;
